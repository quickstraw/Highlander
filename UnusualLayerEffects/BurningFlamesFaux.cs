...
namespace Highlander.UnusualLayerEffects
{
    class BurningFlamesFaux : FauxDust
    {
        ...
        public BurningFlamesFaux(PlayerDrawInfo info, Vector2 offset, string texturePath, float scale) : base(info, offset, texturePath, scale)
        {
            OnSpawn();
        }

        public BurningFlamesFaux(PlayerDrawInfo info, Vector2 offset, Texture2D texture, float scale) : base(info, offset, texture, scale)
        {
            OnSpawn();
        }
